...
# custom_imports에 object_mask_hook 뿐만 아니라 custom_logger_hook도 import하도록 수정
custom_imports = dict(
    imports=[
        'projects.mmdet3d_plugin.core.hook.object_mask_hook',
        'projects.mmdet3d_plugin.core.hook.custom_logger_hook'
    ],
    allow_failed_imports=False
)

# 후크 등록 (여기서 CustomLoggerHook이 이제 HOOKS에 등록됨)
custom_hooks = [
    dict(type='ObjectMaskEpochUpdater')
]
...
model = dict(
    ...
    img_backbone=dict(
        ...
        norm_eval=True,
        with_cp=True, # 원본: True -> False # DDP 에러(RuntimeError: Expected to mark a variable ready only once.)
        style='pytorch'),
    img_neck=dict(
        type='FPN',
...
train_pipeline = [
    ...
    dict(type='Collect3D',
         keys=[ 'img', 'object_mask_gt', 'voxel_semantics', 'mask_lidar', 'mask_camera', 'prev_exists'], # ", 'object_mask_gt'"추가
...
optimizer = dict(
    type='AdamW', 
    lr=5e-5, # 학습 최적화 전략 / Detach: 5e-5, / 논문: 7.5e-5
    paramwise_cfg=dict(
        custom_keys={
...
# 총 1265850 iters 기준, 10회 평가
evaluation = dict(
    interval=126585, # 126585 iters 마다 평가.
    pipeline=test_pipeline,
    metric='mIoU',
    save_best='mIoU' # mIoU 기준으로 best model 저장
)
...
# custom hook 추가. 매 iters 시작 시 ObjectMaskModule의 epoch를 업데이트.
custom_hooks = [
    dict(type='ObjectMaskEpochUpdater')
]
...
resume_from = None
# resume_from = "/home/hyun/local_storage/code/seg_object_vieeew/ViewFormer-Occ/work_dirs/Object_Mask-Seg-Multi_GPU-DDPDEBUG/50vj_iter_635000.pth" 
# resume_from = "/home/hyun/local_storage/code/seg_object_vieeew/ViewFormer-Occ/work_dirs/Object_Mask-Seg-Multi_GPU-DDPDEBUG/100vj_iter_1265000.pth" 