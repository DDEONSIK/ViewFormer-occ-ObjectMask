...
class ViewAttn(BaseModule): #ViewAttn
    """
    Detr3d에 사용되는 Attention 모듈.
    Args:
        embed_dims (int): Attention의 embedding dimension.
                    Default: 256.
        num_heads (int): 병렬 Attention heads.
                    Default: 64.
        num_levels (int): Attention에 사용되는 feature map의 수.
                    Default: 4.
        num_points (int): 각 head의 각 query마다 sampling할 point의 수.
                    Default: 4.
        im2col_step (int): image_to_column에서 사용되는 step.
                    Default: 64.
        dropout (float): inp_residual에 적용되는 Dropout layer.
                    Default: 0..
        init_cfg (obj:mmcv.ConfigDict): 초기화를 위한 Config.
                    Default: None.
    """
    ...
    def forward(self,
        ...
        if self.use_checkpoint and self.training:
            query = cp.checkpoint(
                        ...
                        level_start_index,
                        img_metas,
                        use_reentrant=False # DDP 에러 관련 추가
                        )
        else:
...
class ViewFormerTransformerLayer(BaseModule):
    """비전 트랜스포머를 위한 기본 `TransformerLayer`.

    `mmcv.ConfigDict`로부터 생성될 수 있으며 더 유연한 사용자 정의를 지원.
    예를 들어, 임의의 수의 `FFN 또는 LN`을 사용할 수 있고
    `attn_cfgs`라는 `ConfigDict` 리스트를 지정하여 다양한 종류의 
    `attention`을 사용 가능. `operation_order`의 첫 번째 요소로 
    `norm`을 지정하면 `prenorm`을 지원한다는 점이 중요.
    `prenorm`에 대한 자세한 내용: `On Layer Normalization in the
    Transformer Architecture <https://arxiv.org/abs/2002.04745>`.

    Args:
        attn_cfgs (list[`mmcv.ConfigDict`] | obj:`mmcv.ConfigDict` | None):
            `self_attention` 또는 `cross_attention` 모듈을 위한 설정.
            리스트 내 설정의 순서는 operation_order의 해당 
            attention과 일치해야 함.
            딕셔너리인 경우, operation_order의 모든 attention 모듈이 
            이 설정으로 생성. 기본값: None.
        ffn_cfgs (list[`mmcv.ConfigDict`] | obj:`mmcv.ConfigDict` | None):
            FFN을 위한 설정. 리스트 내 설정의 순서는 operation_order의 
            해당 ffn과 일치해야 함.
            딕셔너리인 경우, operation_order의 모든 attention 모듈이 
            이 설정으로 생성.
        operation_order (tuple[str]): 트랜스포머에서 연산의 실행 순서.
            예: ('self_attn', 'norm', 'ffn', 'norm').
            첫 번째 요소로 `norm`을 지정하면 `prenorm`을 지원.
            기본값: None.
        norm_cfg (dict): 정규화 레이어를 위한 설정 딕셔너리.
            기본값: dict(type='LN').
        init_cfg (obj:`mmcv.ConfigDict`): 초기화를 위한 설정.
            기본값: None.
        batch_first (bool): Key, Query, Value가 
            (batch, n, embed_dim) 또는
            (n, batch, embed_dim) 형태. 기본값: False.
    """
    ...
    def forward(self,
                ...
                key_padding_mask=None,
                **kwargs):
        """TransformerDecoderLayer의 순전파 함수.

        **kwargs는 어텐션의 특정 인자들을 포함.

        Args:
            query (Tensor): 입력 쿼리로 
                self.batch_first가 False인 경우 shape는
                [num_queries, bs, embed_dims]이며, 그렇지 않으면
                [bs, num_queries embed_dims].
            key (Tensor): 키 텐서로 self.batch_first가 False인 경우
                shape는 [num_keys, bs, embed_dims]이며, 그렇지 않으면
                [bs, num_keys, embed_dims].
            value (Tensor): `key`와 동일한 shape를 가진 값 텐서.
            query_pos (Tensor): `query`를 위한 위치 인코딩.
                기본값: None.
            key_pos (Tensor): `key`를 위한 위치 인코딩.
                기본값: None.
            attn_masks (List[Tensor] | None): 해당하는 어텐션 계산에 
                사용되는 2D 텐서. 길이는 `operation_order`의 `attention` 수와 동일해야 함. 
                기본값: None.
            query_key_padding_mask (Tensor): `query`를 위한 ByteTensor로,
                shape는 [bs, num_queries]. `self_attn` 레이어에서만 
                사용. 기본값: None.
            key_padding_mask (Tensor): `query`를 위한 ByteTensor로,
                shape는 [bs, num_keys]. 기본값: None.

        Returns:
           Tensor: 순전파된 결과로 shape는 [num_queries, bs, embed_dims].
        """
