...
class VoxelBevUNet(nn.Module): #
    ...
    def forward(self, x, need_bev_feat=False, temporal_feat=None): #최초 BEV맵 생성

        identity = x

        B, C, Z, H, W = x.shape
        voxel_feat = x.permute(0, 2, 3, 4, 1)
        # 여기서 Z 차원에 대해 max pooling을 수행하여 BEV 특징 맵을 생성
        bev_feat = self.pillar_encoder(voxel_feat).max(dim=1)[0] # voxel feature를 BEV 맵으로 압축 (생성)
        ...
        if need_bev_feat:
            ...
            return out_feat # voxel feature map을 보완한 값. # out_feat는 원래 입력과 residual 연결된 결과
