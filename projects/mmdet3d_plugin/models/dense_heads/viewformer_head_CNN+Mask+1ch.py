...
class ViewFormerHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.object_mask_module = ObjectMaskModule(in_channels=self.bev_dim)
    ...
    def forward(self,
        ...
        # ===== ObjectMaskModule 호출 및 Dummy 연산 추가 =====
        pred_object_mask = self.object_mask_module(bev_feat)  # shape: (B,1,H,W) 1 1 200 200
        # print("     #3 [viewformer_head.py] pred_object_mask 생성됨, shape:", pred_object_mask.shape)


        # === 추가: 모든 object_mask_module 파라미터를 강제로 forward 계산 그래프에 포함 ===
        # dummy_grad = 0.0
        # for p in self.object_mask_module.parameters():
        #     dummy_grad = dummy_grad + p.sum()
        # # 0.0 * dummy_grad를 더하여, 값은 변화시키지 않으면서 모든 파라미터에 대한 연결을 강제함
        # pred_object_mask = pred_object_mask + 0.0 * dummy_grad
        dummy_trigger = sum(p.view(-1)[0] for p in self.object_mask_module.parameters() if p.requires_grad)
        pred_object_mask = pred_object_mask + 0.0 * dummy_trigger

        # --- 추가: Dummy 연산 ---
        # pred_object_mask의 모든 원소의 합에 0을 곱하여(dummy_obj) 
        # 이를 최종 출력에 포함시키면, DDP가 이 텐서를 사용했다고 인식합니다.
        # object_mask_module의 출력이 손실 계산에 반드시 사용되도록 1e-6 배수를 곱합니다.
        outs['dummy_obj'] = pred_object_mask.sum() * 1e-10

        #=== 시각화 코드 ===
        # save_dir = '/home/hyun/local_storage/code/object_vieeew/ViewFormer-Occ/work_dirs/Visualization/Object_Mask-lite_Train_1%' # 시각화 결과 저장 경로

        # # selected_channels = [0, 1, 9, 25, 64, 114, 125] # 시각화할 채널 선택
        # selected_channels = [64]
        # timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        
        # # 1. 결합 전 BEV feature 시각화
        # for ch in selected_channels:
        #     plt.figure(figsize=(10, 10))
        #     plt.imshow(bev_feat[0, ch].detach().cpu().numpy(), cmap='viridis', interpolation='nearest')
        #     plt.colorbar()
        #     plt.xlabel("Width")
        #     plt.ylabel("Height")
        #     plt.title(f'BEV Feature Before Mask - Channel {ch} - run 1%')
        #     plt.savefig(os.path.join(save_dir, f'1-before_bev_feat_ch{ch}_{timestamp}.png'))
        #     plt.close()
        # print("   # [1-before_bev_feat] 시각화 완료.")

        # # 2. Object_Mask-lite (배경 제거 후 객체 강조) 시각화
        # plt.figure(figsize=(10, 10))
        # plt.imshow(highlighted_object_mask[0, 0].detach().cpu().numpy(), cmap='inferno', interpolation='nearest')
        # plt.colorbar()
        # plt.xlabel("Width")
        # plt.ylabel("Height")
        # plt.title('Highlighted Object_Mask-lite (No Background) - run 1%')
        # plt.savefig(os.path.join(save_dir, f'2-highlighted_object_mask_{timestamp}.png'))
        # plt.close()
        # print("   # [2-object_mask] 시각화 완료.")
        #=================================

        #=== Residual 결합 ===
        if pred_object_mask is not None:
            #=== 객체 강조 마스크 (배경 제거) ===
            object_mask_threshold = 0.5  # 임계값 (객체 강조)
            highlighted_object_mask = pred_object_mask * (pred_object_mask > object_mask_threshold).float()
            # [추가됨] Mask 적용 전 채널 수 확인
            # print(f"     [DEBUG] Mask 적용 전 BEV Feature 채널 수: {bev_feat.shape[1]}")
            
            enhanced_bev_feat = torch.cat([bev_feat, highlighted_object_mask], dim=1)  # [B, 127, H, W]
            # [추가됨] Mask 적용 후 채널 수 확인
            # print(f"     [DEBUG] Mask 적용 후 최종 BEV Feature 채널 수: {enhanced_bev_feat.shape[1]}")
        else:
            enhanced_bev_feat = bev_feat.clone()  # 마스크 없을 경우 원본 유지
            # print("     [DEBUG] [viewformer_head.py] 객체 마스크 없음. 원본 BEV Feature 유지.")

            #=================================
            # # 3. 결합 후 BEV feature 시각화
            # for ch in selected_channels:
            #     plt.figure(figsize=(10, 10))
            #     plt.imshow(enhanced_bev_feat[0, ch, :, :].detach().cpu().numpy(), cmap='viridis', interpolation='nearest')
            #     plt.colorbar()
            #     plt.xlabel("Width")
            #     plt.ylabel("Height")
            #     plt.title(f'BEV Feature After Mask - Channel {ch} - run 1%')
            #     plt.savefig(os.path.join(save_dir, f'3-after_bev_feat_ch{ch}_{timestamp}.png'))
            #     plt.close()
        ...
        # #=== 결과 저장 ===
        # outs = {} 
        # → outs = {}로 덮어쓰면 이전에 저장된 outs['bev_embed'], outs['dummy_obj'] 등의 값이 완전히 삭제
        # → 이후 loss()에서 해당 키가 없어서 학습이 비정상적으로 진행되거나 성능이 낮게 나올 수 있음
        outs['pred_object_mask'] = pred_object_mask
        outs['bev_embed'] = enhanced_bev_feat  # 최종 강화된 BEV Feature 저장
    ...
    def loss(self,
        ...
        # === 객체 마스크 Loss 통합 (항상 계산) ===
        pred_mask = preds_dicts.get('pred_object_mask', None)
        if (pred_mask is not None) and (object_mask_gt is not None):
            loss_mask = loss_object_mask(pred_mask, object_mask_gt, loss_weight=1.0)
            loss_dict['loss_object_mask'] = loss_mask
        else:
            # 만약 pred_mask가 존재하지 않으면, 모든 rank에서 동일하게 dummy GT를 사용.
            # (예: pred_mask가 None이면 0 텐서를 생성)
            if pred_mask is None:
                dummy_pred = torch.zeros(1, device=voxel_semantics.device)
            else:
                dummy_pred = pred_mask
            loss_dict['loss_object_mask'] = 1e-3 * torch.mean(dummy_pred)
        # =================================
        ...
        # === 강제 Dummy Loss === 
        # object_mask_module의 모든 파라미터를 이용하여 dummy loss를 계산.
        dummy_loss = torch.zeros(1, device=voxel_semantics.device, requires_grad=True)
        for p in self.object_mask_module.parameters():
            # 각 파라미터의 합을 dummy_loss에 더합니다.
            dummy_loss = dummy_loss + torch.sum(p ** 2)
        # multiplier를 1e-2로 설정 (필요 시 더 크게 조정)
        loss_dict['loss_dummy'] = 1e-10 * dummy_loss
        # =================================
        ...
        # 모든 손실 항목을 계산한 후, 전체 모델 파라미터에 대한 dummy loss 추가. 
        # dummy_loss_all = 0.0
        # for p in self.parameters():
        #     dummy_loss_all = dummy_loss_all + 1e-10 * torch.sum(p ** 2)
        dummy_loss_all = sum(p.view(-1)[0] for p in self.parameters() if p.requires_grad) * 1e-10
        loss_dict['dummy_loss_all'] = dummy_loss_all
