...
class ViewFormer(MVXTwoStageDetector):
    ...
    def extract_img_feat(self, img):
        ...
        if img is not None:
            ...
            img_feats = self.img_backbone(img) #img_backbone 중복 호출 아님 확인.
    ...
    #     # [수정] reshape 하지 않고 그대로 반환
    #     return img_feats  # -> List of 4D tensors, each: [B*T*N, C, H, W]
    ...
    def forward_pts_train(self,
                          ...
                          mask_lidar=None,
                          mask_camera=None,
                          object_mask_gt=None,  # object_mask_gt 추가
                          requires_grad=True,
                          return_losses=False,
                          ):
        ...
        if return_losses:
            loss_inputs = [voxel_semantics, mask_lidar, mask_camera, outs, object_mask_gt]  # object_mask_gt Loss 계산에 추가
    ...
    def window_forward_tarin(self,
        ...
        for i in range(num_frame):
            ...
            loss, space_embed = self.forward_pts_train(img_feats[i],
                                                       ...
                                                       object_mask_gt=object_mask_gt[:, label_idx, ...] if object_mask_gt is not None else None,  # 추가
    ...
    def forward_train(self,
                      ...
                      mask_camera=None,
                      prev_exists=None,
                      object_mask_gt=None):  # 추가: object_mask_gt 인자를 명시적으로 추가 (기본값 None)
        ...
        # # === 1. 분리 및 flatten ===
        # target_img = img[:, -self.num_frame_backbone_grads:, ...]  # [B, T, N_cam, C, H, W]
        # temp_img = img[:, :-self.num_frame_backbone_grads, ...]
        # batch_size, num_sliced_frames, num_cams, C, H, W = target_img.shape

        # # → Flatten 후 extract_feat 호출
        # flat_target_img = target_img.reshape(batch_size * num_sliced_frames * num_cams, C, H, W)
        # flat_target_feats_per_level = self.extract_feat(flat_target_img)  # [BxTxN, C, H, W]
        # print("[DEBUG] shape of flat_target_feats_per_level[0]:", flat_target_feats_per_level[0].shape)


        # # === 2. (B, T, N, ...)로 다시 복원 ===
        # reshaped_feats_per_level = []
        # for lvl_feat in flat_target_feats_per_level:
        #     shape = lvl_feat.shape
        #     if len(shape) != 4:
        #         raise RuntimeError(f"[ERROR] Unexpected shape: {shape}, expected 4D tensor [BxTxN, C, H, W]")

        #     BxTxN, C_feat, H_feat, W_feat = shape
        #     assert BxTxN == batch_size * num_sliced_frames * num_cams, \
        #         f"[ERROR] Unexpected batch dim: got {BxTxN}, expected {batch_size * num_sliced_frames * num_cams}"

        #     lvl_feat_6d = lvl_feat.view(batch_size, num_sliced_frames, num_cams, C_feat, H_feat, W_feat)
        #     reshaped_feats_per_level.append(lvl_feat_6d)


        # # 3. 프레임별로 피처 묶기
        # target_img_feats = [[] for _ in range(num_sliced_frames)]
        # for frame_idx in range(num_sliced_frames):
        #     for lvl_idx in range(len(reshaped_feats_per_level)):
        #         frame_level_feat = reshaped_feats_per_level[lvl_idx][:, frame_idx, :, :, :, :]  # [B, N, C, H, W]
        #         target_img_feats[frame_idx].append(frame_level_feat)

        # # 4. Depth Loss (옵션)
        # if self.depth_supvise:
        #     view_depth_loss_list, view_cls_loss_list = [], []
        #     for i in range(num_sliced_frames):
        #         this_img_metas = [each[-self.num_frame_backbone_grads + i] for each in img_metas]
        #         loss_d, loss_cls = self.view_depth_sem_loss(target_img_feats[i], this_img_metas)
        #         view_depth_loss_list.append(loss_d)
        #         view_cls_loss_list.append(loss_cls)

        # # 5. Temporal frame 병합
        # if self.use_temporal and temp_img.size(1) > 0:
        #     temporal_img_feats = self.inference_img_feat(temp_img, training=True)
        #     img_feats = temporal_img_feats + target_img_feats
        # else:
        #     img_feats = target_img_feats
        ...
        losses, space_embed = self.window_forward_tarin(img_feats,
                                           ...
                                           prev_exists=prev_exists,
                                           object_mask_gt=object_mask_gt) # `window_forward_train()`에 `object_mask_gt` 전달
    ...
    def forward_dummy(self, img): # Flops? FPS? test용 코드임.
        print(f"[DEBUG] Input shape in forward_dummy: {img.shape}")  # 현재 입력 구조 확인
        img_feats = self.extract_feat(img)

        img_metas = [{
            'img_shape': [[224, 224, 3] for _ in range(6)],
            'ego2lidar': np.eye(4),
            'lidar2img': np.stack([np.eye(4)] * 6, axis=0),
            'ego2global': np.eye(4),
            'timestamp': i * 0.1,
            'scene_token': f'dummy_{i}',
        } for i in range(1)]

        print("[DEBUG] Before passing to pts_bbox_head")
        outs = self.pts_bbox_head(img_feats, img_metas=img_metas)
        print("[DEBUG] After pts_bbox_head execution")
        return outs
