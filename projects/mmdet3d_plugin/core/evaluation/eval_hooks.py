...
class CustomDistEvalHook(BaseDistEvalHook):
    ...
    def _do_evaluate(self, runner):
        ...
        if runner.rank == 0:
            ...
            # self.dataloader.dataset.evaluate(results, runner=runner) # 원본 evaluate_miou -> 수정 evaluate
                                                                        # single_gpu는 evaluate_miou라고 되어있는데 
                                                                        # 정상 mIoU 저장 정상 작동. -> 이상함.
            self.dataloader.dataset.evaluate_miou(results, runner=runner) #이것만 활성화. mIoU 저장 정상 작동. 저장 비활성화.
            # if self.save_best:
