...
data_root = '/mnt/dataset_storage/nuscenes'
info_path = '/mnt/dataset_storage/nuscenes/nuscenes_12hz_infos_val.pkl'
...
def worker(info):
    ...
    try:
        lidar_path = info['lidar_infos'][lidar_key]['filename']
        full_lidar_path = os.path.join(data_root, lidar_path)
        points = np.fromfile(full_lidar_path, dtype=np.float32, count=-1).reshape(-1, 5)[..., :4]
        print(f"[INFO] LiDAR 파일 로드 성공: {full_lidar_path}")
    except Exception as e:
        print(f"[ERROR] LiDAR 파일 로드 실패: {full_lidar_path}, 오류: {e}")
        return
    ...
    for i, cam_key in enumerate(cam_keys):
        ...
        #                                 f'{file_name}.bin'))
        try:
            output_path = os.path.join(data_root, 'depth_pano_seg_gt', f'{file_name}.bin')
            np.concatenate([pts_img[:2, :].T, depth[:, None], pts_label_img],
                        axis=1).astype(np.float32).flatten().tofile(output_path)
            print(f"[INFO] 파일 생성 완료: {output_path}")
        except Exception as e:
            print(f"[ERROR] 파일 생성 실패: {output_path}, 오류: {e}")
...
if __name__ == '__main__':
    ...
    for info_idx, info in enumerate(infos):
        ...
        po.apply_async(func=worker, args=(info, ), error_callback=lambda e: 
                        print(f"[ERROR] 병렬 처리 중 오류 발생: {e}"))
